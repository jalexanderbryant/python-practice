...
class SingleLinkedList(object):
    ...
    def push_to_front(self, obj):
        """Prepends new value to the START of the list"""
        new_node = self.__create_new_node(obj)
        if self.begin == None: # Empty list
            self.begin = new_node
            self.tail = new_node
        else:
            new_node.next = self.begin # Set net obj's next to head
            self.begin = new_node
    ...
    def get(self, index):
        ...
        if self.begin == None or index > self._count - 1:
            return None
        # start count at 0
        count = 0
